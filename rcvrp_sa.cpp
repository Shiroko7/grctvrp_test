...
vector<vector<int>> intra_shuffle(vector<vector<int>> sol)
{
    vector<vector<int>> sol_n = sol;
    //intra route
    int route_1 = int_rand(0, sol.size()-1);
    ...
    if (sol_n[route_1].size() > 1)
    {
        random_shuffle(sol_n[route_1].begin(), sol_n[route_1].end());
    }
    return sol_n;
}
